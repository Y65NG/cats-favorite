import type { GameId, MiniGameDefinition, MiniGameResult, RawSignals } from './types'
...
function createTarget(gameId: GameId, width: number, height: number, id: number): Target {
...
export function createEngineState(gameId: GameId): EngineState {
  ...
  const state: EngineState = {
    ...
    pointerDown: (x, y) => {
      ...
      state.reactions.push(Math.max(120, state.elapsedMs - hitTarget.bornAt))
      ...
      if (state.lastHitTargetId !== null && state.lastHitTargetId !== hitTarget.id) {
      ...
      Object.assign(
        hitTarget,
        createTarget(state.gameId, state.width, state.height, state.nextTargetId),
      )
...
function stepLaserTarget(state: EngineState, target: Target, deltaSeconds: number) {
...
function stepFishTarget(state: EngineState, target: Target, deltaSeconds: number) {
  if (state.elapsedMs >= target.laneSwitchAt) {
    ...
    const nextLane =
      candidateLanes[(target.id + Math.floor(state.elapsedMs / 240)) % candidateLanes.length]
  ...
  target.x += target.vx * deltaSeconds
  target.anchorY += (target.laneTargetY - target.anchorY) * Math.min(1, deltaSeconds * 3.4)
...
function stepFeatherTarget(target: Target, deltaSeconds: number) {
  ...
  target.y =
    target.anchorY - Math.cos(target.wobble) * 84 + Math.cos(target.wobble * 2) * 14
}
...
function normalizeSignals(state: EngineState) {
  ...
  const effectiveDurationMs = Math.max(6000, Math.min(state.elapsedMs || state.durationMs, state.durationMs))
  const averageReaction =
    state.reactions.length > 0
      ? state.reactions.reduce((sum, reaction) => sum + reaction, 0) / state.reactions.length
      : 1200
  ...
  const reaction = clamp((1100 - averageReaction) / 780 + accuracy * 0.18, 0, 1)
  const focus = clamp(state.focusTimeMs / effectiveDurationMs + accuracy * 0.22, 0, 1)
  const ambush = clamp(edgeRate * 0.62 + reaction * 0.24 + switchRate * 0.18, 0, 1)
  const tracking = clamp(accuracy * 0.42 + switchRate * 0.24 + streakRate * 0.34, 0, 1)

  if (state.gameId === 'laser') {
    return { reaction: clamp(reaction + 0.1, 0, 1), focus, ambush, tracking }
  }
...
function drawHitEffect(ctx: CanvasRenderingContext2D, effect: HitEffect) {
  ...
  if (effect.kind === 'fish') {
    ...
    for (const offset of [0, 18]) {
      ctx.beginPath()
      ctx.arc(effect.x, effect.y, 12 + progress * 34 + offset * progress * 0.2, 0, Math.PI * 2)
      ctx.stroke()
    }
    ctx.restore()
    return
  ...
  if (effect.kind === 'feather') {
    ...
    for (let index = 0; index < 5; index += 1) {
      ...
      ctx.beginPath()
      ctx.ellipse(x, y, 10 - progress * 3, 4.8 - progress * 1.4, angle, 0, Math.PI * 2)
      ctx.fill()
    }
    ctx.restore()
    return
...
function drawTarget(ctx: CanvasRenderingContext2D, state: EngineState, target: Target) {
  if (!target.visible) {
    return
  }
  ...
  if (state.gameId === 'laser') {
    ...
    ctx.fillStyle = target.hue
    ctx.beginPath()
    ctx.ellipse(0, 0, target.radius * 1.1, target.radius * 0.7, 0, 0, Math.PI * 2)
    ctx.fill()
    ctx.beginPath()
...
export function drawGameFrame(
  ...
  context.fillRect(28, 24, state.width - 56, 52)
  context.fillStyle = '#5C4630'
  context.font = '700 20px "Avenir Next", "PingFang SC", sans-serif'
  context.fillText(`${definition.name} · 看准时机就拍`, 44, 56)
